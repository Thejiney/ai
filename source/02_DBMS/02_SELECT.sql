...
FROM EMP; -- EMP테이블의 EMPNO, ENAME, SAL, JOB 필드만 모든 행
SELECT EMPNO, ENAME, COMM
FROM EMP;

SELECT EMPNO AS "사 번", ENAME AS "이름", SAL   AS "급여"
FROM EMP; -- 필드명에 별칭을 사용
SELECT EMPNO "사 번", ENAME "이름", SAL   "급여"
FROM EMP; -- 별칭에 as 생략 가능
SELECT EMPNO NO, ENAME NAME, SAL   SALARY
FROM EMP; -- 별칭에 SPACE가 없는 경우 " 생략가능
SELECT EMPNO "NO", ENAME "NAME", SAL   "SALARY"
FROM EMP;
...
ORDER BY 연봉;             -- (4) ORDER BY 절에는 필드의 별칭 사용 가능
...
FROM EMP;
    -- EX. 모든 사원의 ENAME, SAL(급여), COMM(상여), 연봉(SAL*12 + COMM) 출력
SELECT ENAME, SAL, COMM, SAL * 12 + COMM
FROM EMP; -- 산술연산결과는 NULL을 포함하면 결과도 NULL
    -- NVL(널일수도있는값, 널이면대체할값) : 널일수도있는값과 널이면대체할값의 타입이 일치
SELECT ENAME, SAL, COMM, SAL * 12 + NVL(
    COMM, 0
) ANNUALSAL
FROM EMP;
    -- MGR이 널이면 'CEO'라고 출력
SELECT EMPNO, ENAME, NVL(
    TO_CHAR(MGR), 'CEO'
) MGR
FROM EMP;
...
FROM EMP
WHERE HIREDATE > TO_DATE('81/02', 'RR/MM');
...
FROM EMP
WHERE ENAME LIKE '%N%';
    -- EX. 이름에 N이 들어가거나 JOB에 N이 들거나는 사원의 모든 필드
SELECT *
FROM EMP
