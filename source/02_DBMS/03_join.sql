...
FROM EMP  E, DEPT D
WHERE E.DEPTNO = D.DEPTNO AND HIREDATE BETWEEN '82/01/01' AND '82/12/31' AND E.DEPTNO IN ( 10, 20 )
ORDER BY SAL;

SELECT ENAME, SAL, LOC
FROM EMP  E, DEPT D
WHERE E.DEPTNO = D.DEPTNO AND HIREDATE LIKE '82/__/__' AND E.DEPTNO IN ( 10, 20 )
ORDER BY SAL;
...
SELECT ENAME, SAL, GRADE, ( SAL + NVL(
    COMM, 0
) ) * 12 ANNUALSAL, DNAME
FROM EMP  E, DEPT D, SALGRADE
...
FROM EMP  E, DEPT D, SALGRADE
WHERE E.DEPTNO = D.DEPTNO AND SAL BETWEEN LOSAL AND HISAL AND HIREDATE LIKE '81/__/__'
ORDER BY GRADE DESC;
...
INSERT INTO EMP VALUES (
    9999, '홍길동', NULL, NULL, NULL, 3000, NULL, 50
);
...
WHERE E1.DEPTNO = D1.DEPTNO AND E1.ENAME = 'SCOTT' AND E2.DEPTNO = D2.DEPTNO AND E2.ENAME != 'SCOTT' AND D1.LOC = D2.LOC;
...
--3. 이름, 급여, 업무, 직속상사명 . (상사가 없는 직원까지 전체 직원 다 출력.
    --상사가 없을 시 '없음'으로 출력)
SELECT W.ENAME, W.SAL, W.JOB, NVL(
    M.ENAME, '없음'
) MANAGER
FROM EMP W, EMP M
WHERE W.MGR = M.EMPNO (+);
...
SELECT W.ENAME, W.SAL, W.DEPTNO, D.DNAME, D.LOC, NVL(
    M.ENAME, '없음'
) MANAGER
FROM EMP  W, DEPT D, EMP  M
WHERE W.DEPTNO = D.DEPTNO AND W.MGR = M.EMPNO (+);
...
SELECT W.ENAME, W.SAL, GRADE, D.DNAME, ( W.SAL + NVL(
    W.COMM, 0
) ) * 12 ANNUALSAL, M.ENAME  MANAGER
FROM EMP  W, SALGRADE, DEPT D, EMP  M
WHERE W.SAL BETWEEN LOSAL AND HISAL AND W.DEPTNO = D.DEPTNO AND W.MGR = M.EMPNO;
