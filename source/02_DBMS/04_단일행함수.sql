...
FROM
	...
	SELECT
		concat(concat(ename, '는 '), concat(job, '다'))
	FROM
		emp;
...
FROM
	...
	FROM
		emp
	WHERE
		substr(hiredate, 4, 2) = '09'; -- (DATE)날짜형도 가능
...
--(3) MONTHS_BETWEEN(날짜형1, 날짜형2) : 두 날짜형 데이터 간 개월수 (날짜형1이 큰 시점)
    ...
    -- EX. 수료시점까지 남은 개월수
	SELECT
		trunc(months_between(TO_DATE('25/03/07 18:00', 'RR/MM/DD HH24:MI'), sysdate), 2)
	FROM
		dual;
...
FROM
	...
	INSERT INTO 
		emp
	VALUES
		(9999,'홍길동',NULL,NULL,'80/08/31',900,NULL, 40);
...
-- (7) ROUND(날짜, xx) : 날짜 반올림(XX:'YEAR', 'MONTH', 'DAY', TODFIR)
	...
	SELECT
		to_char(round(sysdate), 'MM/DD HH24:MI')
	FROM
		dual; -- 가까운 0시 0분
	...
	SELECT
		to_char(trunc(sysdate), 'MM/DD HH24:MI')
	FROM
		dual; -- 오늘 0시 0분
	SELECT
		trunc(sysdate, 'DAY')
...
-- (1) TO_CHAR(날짜형, '출력형식')
	...
	SELECT
		ename,
		to_char(hiredate, 'RR"년"MM"월"DD"일" DY"요일" AM HH"시"MI"분"SS"초"')
	FROM
		emp;
-- (2) TO_CHAR(숫자, '출력형식')
...
-- (3) TO_DATE(문자, '패턴')
	...
	FROM
		emp
	WHERE
		to_char(hiredate, 'RR/MM/DD') BETWEEN '81/05/01' AND '83/05/01';
