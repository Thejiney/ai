-- [V] 그룹함수 : SUM, AVG, MIN, MAX, STDDEV(표준편차), VARIANCE(분산)
SELECT
	MAX(SAL)
FROM
	EMP;
...
FROM
	...
	FROM
		EMP;
	-- 탄탄EX4. 10번부서 소석의 사원 중에 COMM을 받는 사원의 수(COUNT함수 사용)
	SELECT 
		COUNT(*)
	FROM 
		EMP
...
-- ★ 2. GROUP BY 절 : 그룹함수를 쓰되, 어떤 컬럼 기준으로 그룹함수를 적용
	...
	SELECT
		...
		ROUND(STDDEV(SAL),2) SAL_STDDEV
	FROM
		EMP
	GROUP BY -- GROUP BY 절은 필드의 별칭 사용 불가
		DEPTNO 
	ORDER BY -- ORDER BY 절은 필드의 별칭 사용 가능
		NO;
...
-- ★ 3. HAVING 절 : 그룹함수의 결과를 제한할 때
	...
	WHERE
		SAL < 5000
	GROUP BY	-- GROUP BY 절에 필드 별칭 사용 불가
		DEPTNO
	HAVING		-- HAVING 절에 필드 별칭 사용 불가
		AVG(SAL) >= 1800
	ORDER BY
		AVG_SAL;
...
--부서번호별
SELECT
	DEPTNO, COUNT(*), ROUND(AVG(SAL),2), MIN(SAL), MAX(SAL),
	SUM(SAL)
FROM
	EMP
GROUP BY
...
-- 부서명 그룹
SELECT
	DNAME, ROUND(AVG(SAL),2) "AVG_SAL", SUM(SAL), MIN(SAL)
FROM
	EMP E, DEPT D
WHERE
	E.DEPTNO = D.DEPTNO
...
SELECT
	TO_CHAR(HIREDATE,'RR') "YEAR", COUNT(*), MIN(SAL), MAX(SAL), AVG(SAL),
	SUM(SAL)
FROM
	EMP
GROUP BY
...
--  total    14	
SELECT
	NVL(TO_CHAR(HIREDATE,'YYYY'),'total') "YEAR", COUNT(*) "CNT"
FROM
	EMP
GROUP BY
