...
-- 서브쿼리 종류(1) 단일행 서브쿼리: 서브쿼리 결과가 단일행 : =, >, >=, <, <=, !=
	-- EX. SCOTT과 동일한 부서번호에서 근무하는 사원의 이름
	SELECT DEPTNO
	FROM EMP
	WHERE ENAME='SCOTT';
...
-- ★ 2. 단일행 서브쿼리
	...
	WHERE E1.HIREDATE = (SELECT MIN(HIREDATE)
						 FROM EMP) AND
		  E2.HIREDATE = (SELECT MAX(HIREDATE)
						 FROM EMP);
	...
	WHERE DEPTNO=(SELECT DEPTNO
				  FROM DEPT
				  WHERE LOC='DALLAS'); -- 서브쿼리 이용
					
	SELECT ENAME, E.DEPTNO
	FROM EMP E, DEPT D
	WHERE E.DEPTNO=D.DEPTNO AND
		  LOC='DALLAS'; -- EQUI JOIN 이용
	...
	SELECT ENAME, SAL, SAL-(SELECT ROUND(AVG(SAL))
							FROM EMP) "GAP"
	FROM EMP;
...
-- (2) ANY(=SOME) : 서브쿼리 결과가 하나라도 만족하면 참
	...
	SELECT SAL
	FROM EMP
	WHERE DEPTNO = 30; -- 950, 1250, 1500, 1600, 2850 다중행서브쿼리
	SELECT *
	FROM EMP
...
-- (4) EXISTS : 서브쿼리 결과가 존재하면 참
	...
	WHERE EXISTS (SELECT *
				  FROM EMP
				  WHERE MGR=MANAGER.EMPNO); -- 서브쿼리 이용
	-- 직속부하가 없는 직원들의 사번, 이름, 급여
	SELECT EMPNO, ENAME, SAL
	FROM EMP MANAGER
	WHERE NOT EXISTS (SELECT *
					  FROM EMP
					  WHERE MGR=MANAGER.EMPNO);
	SELECT M.EMPNO, M.ENAME, M.SAL
	FROM EMP W, EMP M
	WHERE W.MGR(+)=M.EMPNO AND
		  W.ENAME IS NULL;

-- 탄탄1. 부서별로 가장 급여를 많이 받는 사원의 모든 정보를 출력(IN)
-- 부서번호별 가장 큰 급여인사원
...
WHERE E.DEPTNO=D.DEPTNO AND SAL BETWEEN LOSAL AND HISAL AND
	  (E.DEPTNO,SAL) IN (SELECT E.DEPTNO, MAX(SAL)
						 FROM EMP
						 GROUP BY E.DEPTNO)
ORDER BY DEPTNO;
...
SELECT CEIL(EXTRACT(MONTH FROM HIREDATE)/3) QUARTER -- 분기
FROM EMP;
SELECT CEIL(TO_CHAR(HIREDATE, 'MM')/3) QUARTER -- 분기
FROM EMP;
SELECT TO_CHAR(HIREDATE, 'Q') QUARTER -- 분기
FROM EMP;
...
SELECT TO_CHAR(HIREDATE, 'Q') QUARTER, EMPNO, ENAME, JOB, MGR,
       HIREDATE, SAL, COMM
	FROM EMP
...
SELECT ENAME, SAL
  ...
  WHERE EXISTS(SELECT *
				 FROM EMP
				 WHERE E.MGR = EMPNO AND INITCAP(ENAME)='King');

SELECT ENAME, SAL
  FROM EMP
  WHERE MGR = (SELECT EMPNO
				 FROM EMP
				 WHERE INITCAP(ENAME)='King');				 
...
SELECT ENAME, JOB
  ...
  WHERE EMPNO IN (SELECT EMPNO 
				   FROM EMP E, DEPT D
				   WHERE E.DEPTNO=D.DEPTNO AND UPPER(DNAME) = 'SALES');
...
SELECT ENAME, JOB, SAL
  ...
  WHERE SAL IN(SELECT SAL
				 FROM EMP
				 WHERE UPPER(ENAME) = 'SCOTT' OR UPPER(ENAME) = 'WARD');
...
SELECT ENAME, JOB
  ...
  WHERE JOB IN(SELECT JOB
				 FROM EMP E, DEPT D
				 WHERE E.DEPTNO=D.DEPTNO AND UPPER(LOC)='CHICAGO'
				 GROUP BY JOB);
...
SELECT EMPNO, ENAME, SAL, E.DEPTNO
  FROM EMP E, (SELECT DEPTNO, AVG(SAL) AVGSAL
		         FROM EMP
			     GROUP BY DEPTNO) G
  WHERE E.DEPTNO=G.DEPTNO AND
	    SAL>=AVGSAL; -- EQUI JOIN과 FROM 절의 서브쿼리를 이용
...
SELECT DEPTNO, ENAME, SAL
  FROM EMP E, (SELECT JOB, AVG(SAL) AVGSAL
		         FROM EMP
		         GROUP BY JOB) G
  WHERE E.JOB = G.JOB AND 
	    SAL < AVGSAL;
