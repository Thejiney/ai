...
CREATE TABLE EMP02 AS
	SELECT *
	FROM EMP; -- 서브쿼리결과로 EMP02테이블 생성(제약조건 미포함)
...
CREATE TABLE EMP04 AS-- EMP의 특정 행(서브쿼리의 WHERE 절로 제한)
		SELECT *
		FROM EMP
		WHERE DEPTNO=10;
...
CREATE TABLE EMP05 AS -- EMP 구조만 추출(데이터는 추출하지 않고 구조만)
		SELECT *
		FROM EMP
		WHERE 0=1;
...
-- (2) 필드 수정(MODIFY)
ALTER TABLE EMP03
MODIFY EMPNO VARCHAR2(4); -- 숫자데이터가 들어있으면 숫자로만 변환 가능

ALTER TABLE EMP03
MODIFY EMPNO NUMBER(5);

ALTER TABLE EMP03
MODIFY EMPNO NUMBER(4); -- 숫자는 줄이는게 불가능

ALTER TABLE EMP03
MODIFY ENAME VARCHAR2(100);

ALTER TABLE EMP03
MODIFY ENAME VARCHAR2(10); -- 문자데이터필드는 늘이거나 줄이는 것이 가능

SELECT MAX(LENGTH(ENAME)) FROM EMP; -- ENAME 글자수 최대값

ALTER TABLE EMP03
MODIFY ENAME VARCHAR2(6); -- 가능

ALTER TABLE EMP03
MODIFY ENAME VARCHAR2(5); -- 불가능(데이터보다 작은 자리수)
...
ALTER TABLE EMP03
DROP COLUMN ENAME; -- 데이터까지 삭제(취소 불가)
...
INSERT INTO DEPT01
VALUES (60, 'SALES', NULL); -- 명시적 NULL 데이터 추가

INSERT INTO DEPT01 (DEPTNO, DNAME, LOC)
VALUES (70, 'RESEARCH', '신림');

INSERT INTO DEPT01 (LOC, DNAME, DEPTNO)
VALUES ('신길','IT',80);

INSERT INTO DEPT01 (DEPTNO, DNAME)
VALUES (90, 'OPERATION'); -- 묵시적으로 NULL 추가
...
INSERT INTO BOOK (BOOKID, BOOKNAME, RADATE, PRICE)
VALUES (12, '스포츠 과학', NULL, SYSDATE, 50000);
...
CREATE TABLE EMP01 AS
	...
	COMMIT;	
	-- EX. EMP에서 SCOTT의 부서번호는 30, JOB은 MANAGER, SAL과 COMM은 500$t씩 인상,
			-- 상사를 'KING'으로 수정
	UPDATE EMP
	SET DEPTNO = 30,
		JOB = 'MANAGER',
...
SELECT * FROM EMP01;
DELETE FROM EMP01; -- DML 명령어는 취소 가능(DDL은 취소 불가)
SELECT * FROM EMP01;
ROLLBACK;
...
INSERT INTO MY_DATA (ID,NAME,USERID,SALARY)
VALUES(5,'Good','ggood',10000);
--3 TO_CHAR 내장 함수를 이용하여 입력한 자료를 위의 도표와 같은 형식으로 출력하는 SQL문을 작성
SELECT ID, INITCAP(NAME), LOWER(USERID), TO_CHAR(SALARY, '99,999.00')
FROM MY_DATA;
--4 자료를 영구적으로 데이터베이스에 등록하는 명령어를 작성
COMMIT;
--5 ID가 3번인 사람의 급여를 65000.00으로 갱신하고 영구적으로 데이터베이스에 반영
UPDATE MY_DATA
SET SALARY = 65000.00
WHERE ID = 3;
COMMIT;
--6 NAME이 Ford인 사람을 삭제하고 영구적으로 데이터베이스에 반영
DELETE FROM MY_DATA
...
VALUES (40, '신림', 'IT'); -- PRIMARY KEY 제약조건
...
VALUES(1002, 'MANAGER', 10); -- NOT NULL 오류
...
GRANT ALL ON EMP TO scott2; -- EMP 테이블에 모든 권한 부여
SELECT * FROM EMP;
REVOKE ALL ON EMP FROM scott2; -- EMP테이블에 모든 권한 박탈
...
INSERT INTO BOOKCATEGORY(CATEGORYCODE, CATEGORYNAME, OFFICE_LOC)
...
SELECT BOOKNO, BOOKNAME, PUBLISHER, CATEGORYNAME, OFFICE_LOC
