...
CREATE OR REPLACE VIEW EMPv0 AS
...
INSERT INTO EMPv0 VALUES (1111, '홍', 'MANAGER', NULL, NULL, 40);
SELECT * FROM EMPv0;

CREATE OR REPLACE VIEW EMPv0 AS -- EMP테이블에서 특정 행(ROW)
	SELECT *
	FROM EMP
	WHERE DEPTNO = 30;
SELECT * FROM EMPv0;
...
-- VIEW의 제한 조건
-- WITH CHECK OPTION 추가 : 뷰의 조건에 해당되는 데이터마 삽입, 수정, 삭제가 가능
...
INSERT INTO EMPv0 (EMPNO, ENAME, DEPTNO)
VALUES(1112, '박', 40); -- VIEW 제한조건으로 에러
...
CREATE OR REPLACE VIEW EMPv0 AS
	SELECT EMPNO, ENAME, SAL*12 YEAR_SAL -- 별칭을 필드
	FROM EMP
	WHERE DEPTNO = 10;
	
CREATE OR REPLACE VIEW EMPv0 (NO, NAME, YEAR_SAL) AS -- 별칭들만 따로
...
INSERT INTO EMPv0 VALUES (1111, 'LEE', 1200); -- 복합뷰는 INSERT 불가
	...
	-- EX1. 사번, 이름, 급여를 10의 자리에서 반올림한 급여를 뷰로 생성(EMPv0) 1250->1300
	CREATE OR REPLACE VIEW EMPv0 AS
		SELECT EMPNO, ENAME, ROUND(SAL,-2) SAL
		FROM EMP;
	...
	-- EX3. 부서번호, 최소급여, 최대급여, 부서평균 (소수점 1자리에서 반올림)을 포함햔 뷰로 생성(EMPv0)
	CREATE OR REPLACE VIEW EMPv0 AS
		SELECT DEPTNO, MIN(SAL) MIN, MAX(SAL) MAX, ROUND(AVG(SAL),1) AVG
		FROM EMP
		GROUP BY DEPTNO;
		
	-- EX3. 부서명, 최소급여, 최대급여, 부서평균 (소수점 1자리에서 반올림)을 포함햔 뷰로 생성(EMPv0)
	CREATE OR REPLACE VIEW EMPv0 AS
		SELECT DNAME, MIN(SAL) MIN, MAX(SAL) MAX, ROUND(AVG(SAL),1) AVG
		FROM EMP E, DEPT D
		WHERE E.DEPTNO = D.DEPTNO
		GROUP BY DNAME;
...
-- WHERE 조건
	...
	SELECT EMPNO, ENAME, SAL, G.DEPTNO, G.AVG
	FROM EMP E, (
		SELECT DEPTNO, ROUND(AVG(SAL),1) AVG
		FROM EMP
		GROUP BY DEPTNO
	) G
