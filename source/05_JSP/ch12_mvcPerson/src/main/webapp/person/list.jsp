...
<body>
	...
	<c:if test="${not empty deleteMsg }">
		<script>
			alert("${param.id}번 " + "${deleteMsg}");
		</script>
	</c:if>
	<table>
		<tr>
			<th colspan="4"><a href="${conPath }/">처음으로</a>&nbsp; <a
				href="${conPath }/insert.do">지인 추가</a></th>
		</tr>
		<tr>
			<th>ID</th>
			<th>이름</th>
		...
		<c:if test="${empty personList || personList.size() eq 0 }">
			<tr>
				<td colspan="4">해당 데이터가없습니다</td>
			</tr>
		</c:if>
		<c:forEach var="person" items="${personList }">
			<tr
				onclick="location.href='${conPath}/info.do?id=${person.id }&pageNum=${pageNum }'">
				<td>${person.id }</td>
				<td>${person.name }</td>
				<td>${person.tel }</td>
				<td><fmt:formatDate value="${person.birth }" pattern="MM월 dd일" />
				</td>
			</tr>
		</c:forEach>
	</table>
