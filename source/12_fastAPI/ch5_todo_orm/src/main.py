...
async def delete_todo_handler(todo_id:int):
  ...
  if result:
    return result
  raise HTTPException(status_code=404, detail='예외 페이지로 감')

@app.get('/update/{id}')
async def get_update_todo_handler(request:Request, id:int):
  todo = get_todo(id)
  if todo:
    return templates.TemplateResponse('update.html', {'request':request, 'todo':todo})
